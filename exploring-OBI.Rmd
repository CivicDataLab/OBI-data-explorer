...
<br /> Last updated: `r format(Sys.time(), '%d %B, %Y')` (See [changelog](https://github.com/CivicDataLab/OBI-data-explorer/commits/master)).
...
packages_required <- c("ckanr","stringr","dplyr","DT","listviewer","ggplot2","ggalt", "emo", "purrr")
...
### Step 2 - `r emo::ji("money")` Exploring the Union Budget 2020 datasets

All the datasets under released under Union Budget 2020 can be identified using the `union budget 2020` tag. LEt's check all the machine readable `CSV` datasets

```{r union budget 2020 datasets, echo=TRUE, message=FALSE, warning=FALSE}

all_UB_2020_packages <- tag_show(id = 'union budget 2020', include_datasets = T)$packages

all_UB_2020_resources <- purrr::map(all_UB_2020_packages,`[`,'resources')

resource_master <- c()
for(i in 1:length(all_UB_2020_resources)){
  resource_master <- c(resource_master, all_UB_2020_resources[[i]]$resources)
}

all_UB_2020_datasets <- purrr::map(resource_master, `[`,c('name', 'description', 'format', 'url')) %>% dplyr::bind_rows()


DT::datatable(
  all_UB_2020_datasets[all_UB_2020_datasets$format == 'CSV',],
  filter = 'top',
...
### Step 3 - `r emo::ji("building")` Exploring an organisation
...
### Step 4 - `r emo::ji("file")` Exploring datasets present within an organisation
...
### Step 5 - `r emo::ji("e-mail")` Viewing a package
...
### Step 6 - `r emo::ji("record")` Reading data from a file
...
### Step 7 - `r emo::ji("cook")` Preparing data for analysis
...
### Step 8 - `r emo::ji("research")` Exploring processed dataset
...
### Step 9 - `r emo::ji("spy")` Further exploration 
...
```{r bottom dumbell, echo=FALSE, message=FALSE, warning=FALSE}
